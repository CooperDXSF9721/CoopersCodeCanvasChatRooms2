...
async function setupCameraForRoom(roomId) {
  ...
  allCamerasRef.on('child_added', async snapshot => {
    ...
    if (sessionId !== userSessionId && data.enabled && (cameraEnabled || microphoneEnabled)) {
...
async function toggleCamera() {
  ...
  if (cameraEnabled) {
    try {
      // Get video with existing audio if mic is enabled
      const constraints = {
        video: { width: 640, height: 480 },
        audio: microphoneEnabled
      };
      
      // If we already have a stream with audio, we need to get a new one with video
      if (localStream && microphoneEnabled) {
        localStream.getTracks().forEach(track => track.stop());
      }
      ...
      if (snapshot.exists()) {
        ...
        for (const [sessionId, data] of Object.entries(cameras)) {
          if (sessionId !== userSessionId && data.enabled && !peerConnections.has(sessionId)) {
    ...
    // Remove video track but keep audio if mic is enabled
    if (localStream) {
      ...
      // If mic is still enabled, keep the stream with just audio
      if (!microphoneEnabled) {
        localStream = null;
        // Close all peer connections if both are disabled
        peerConnections.forEach((pc, sessionId) => {
          closePeerConnection(sessionId);
        });
      }
...
async function toggleMicrophone() {
  ...
  if (microphoneEnabled) {
    try {
      // Get audio with existing video if camera is enabled
      const constraints = {
        video: cameraEnabled ? { width: 640, height: 480 } : false,
        audio: true
      };
      
      // If we already have a stream with video, we need to get a new one with audio
      ...
      // Create peer connections with all other users who have cameras/mics enabled
      const snapshot = await allCamerasRef.once('value');
      if (snapshot.exists()) {
        const cameras = snapshot.val();
        for (const [sessionId, data] of Object.entries(cameras)) {
    ...
    // Remove audio track but keep video if camera is enabled
    if (localStream) {
      ...
      // If camera is still enabled, keep the stream with just video
      if (!cameraEnabled) {
        localStream = null;
        // Close all peer connections if both are disabled
        peerConnections.forEach((pc, sessionId) => {
...
async function updateCameraDisplay() {
  ...
  Object.entries(cameras).forEach(([sessionId, data]) => {
    ...
    if (data.enabled || data.micEnabled) {
      ...
      // If only mic is enabled (no camera), show black screen with mic icon
      if (!data.enabled && data.micEnabled) {
...
window.addEventListener('load', () => {
  ...
  // Camera and Microphone event listeners
